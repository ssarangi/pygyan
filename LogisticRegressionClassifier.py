...
class LogisticRegressionClassifier(RegressionClassifier):
    def __init__(self, num_variables, learning_rate, threshold):
        RegressionClassifier.__init__(num_variables=num_variables, learning_rate=learning_rate, threshold=threshold)
        self.num_vars = 1
    ...
    def compute_func(self, theta, x):
        ...
        exp_pow = np.sum(per_element_sum)

        result = self.sigmoid(exp_pow)
        return result
    ...
    def compute_cost_func(self, theta):
        ...
        for i, ith_training_data in enumerate(self.training_data):
            ...
            hypothesis_val = self.compute_func(theta, x_i)
            cost += (-y_i * math.log10(hypothesis_val) - ((1 - y_i) * math.log10(1 - hypothesis_val)))

        cost = (1 / (self.num_training_data)) * cost
        return cost
